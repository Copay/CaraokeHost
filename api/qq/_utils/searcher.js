...
export default async function searcher(name, singer, album, length, pageNo = 1, pageSize = 10) {
    ...
    if (length)
        res = res.filter(a => Math.abs(a.interval - length / (10 ** 3)) < 1);
    if (singer)
        res = res.map(a => ({ singerLikelihood: dc(singer, a.singer.map(a => a.name).join(" ")), ...a }));
    if (album)
        res = res.map(a => ({ albumLikelihood: dc(album, a.albumname), ...a }));
